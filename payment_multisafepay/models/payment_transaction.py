...
class PaymentTransaction(models.Model):
    ...
    def _send_refund_request(self, amount_to_refund=0.0):
        ...
        try:
            ...
            _logger.error(
                "API error retrieving order %s: %s", self.reference, str(api_error)
            )
            raise UserError(
                _(
        ...
        if hasattr(order_data, "amount_refunded") and order_data.amount_refunded:
            _logger.warning(
                "Order %s has already been partially refunded", self.reference
            )
        ...
        try:
            ...
            is_bnpl = (
                self.payment_method_code.removeprefix(const.PAYMENT_METHOD_PREFIX)
            ...
            try:
                    ...
                    refund_payload = (
                        ...
                        .add_description(
                            Description(**{}).add_description(
                                f"Refund for Odoo order {self.reference}"
                            )
                        )
                    )
                ...
                refund_response = order_manager.refund(self.reference, refund_payload)
            except Exception:
        ...
        # Create refund transaction (Odoo 18 flow)
        refund_tx = super()._send_refund_request(amount_to_refund=amount_to_refund)

        # Update with MultiSafepay reference and process notification
        notification_data = {
            "status": "completed",
