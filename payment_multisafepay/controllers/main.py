...
class MultiSafepayController(http.Controller):
    ...
    def multisafepay_return(self, **kwargs):
        ...
        if payment_transaction.payment_method_code.removeprefix(PAYMENT_METHOD_PREFIX) in PAYMENT_METHOD_PENDING:
            _logger.info("Payment method '%s' requires manual confirmation. Setting as pending.", 
            ...
            return self._redirect_pending_payment(payment_transaction, order, transactionid)
        ...
        try:
            ...
            try:
                ...
                if not validated:
                    ...
                    payment_transaction._set_error("Webhook validation failed: Invalid signature")
    ...
    def _redirect_to_payment_with_error(self, error_code, message):
        ...
        """
        return request.redirect(f"/payment/status?error={error_code}&message={quote_plus(message)}")
    ...
    def _get_order(self, url, payment_transaction) -> Order:
        ...
        currency, amount, currency_id = self._prepare_amount_and_currency(payment_transaction)

        # Get partners from transaction (invoice/shipping addresses)
        partner_invoice, partner_shipping, sale_order, sale_orders = self._get_partners_from_transaction(payment_transaction)
        ...
        environ = getattr(request.httprequest, 'environ', {}) if request and hasattr(request, 'httprequest') else {}
        partner_invoice_ip_address_raw = environ.get('REMOTE_ADDR', None) if isinstance(environ, dict) else None
        partner_invoice_forwarded_ip_raw = environ.get('HTTP_X_FORWARDED_FOR', None) if isinstance(environ, dict) else None
        partner_invoice_forwarded_ip_raw = partner_invoice_forwarded_ip_raw or partner_invoice_ip_address_raw
        partner_invoice_referrer = environ.get('HTTP_REFERER', None) if isinstance(environ, dict) else None

        user_agent_string = None
        if request and hasattr(request, 'httprequest') and hasattr(request.httprequest, 'user_agent'):
            user_agent = request.httprequest.user_agent
            user_agent_string = getattr(user_agent, 'string', None) if user_agent else None
        ...
        if not sale_orders or sale_orders and use_delivery_as_billing:
            ...
            delivery = (Delivery(**{}).add_first_name(partner_invoice_data['first_name'])
                .add_last_name(partner_invoice_data['last_name'])
                .add_phone(partner_invoice_data['phone_raw'])
                .add_email(partner_invoice_data['email_raw'])
                .add_address1(partner_invoice_data['address1'])
        ...
        if invoice_ids and len(invoice_ids) > 0:
            ...
            order_lines = [
                line for line in all_invoice_lines
                if getattr(line, 'display_type', None) not in ('line_section', 'line_subsection', 'line_note')
            ]
            
            if order_lines:
                source_type = 'invoice'
                _logger.debug("Using invoice lines for cart: %d lines (filtered from %d total)", 
                            len(order_lines), len(all_invoice_lines))
        ...
        if not order_lines and sale_order:
            ...
            order_lines = [
                line for line in all_sale_lines
                if getattr(line, 'display_type', None) not in ('line_section', 'line_subsection', 'line_note')
            ]
            
            if order_lines:
                source_type = 'sale_order'
                _logger.debug("Using sale order lines for cart: %d lines (filtered from %d total)", 
                            len(order_lines), len(all_sale_lines))
