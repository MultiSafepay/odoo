...
class PosPaymentMethod(models.Model):
    ...
    def _get_multisafepay_cloud_sdk(self):
        ...
        credentials for Cloud POS calls and the account key for account-level calls.
    ...
    def _api_create_cloud_pos_order(self, data):
        ...
        def build_create_error_payload(detail, error=None):
            payload = _ErrorPayload.build(
                self,
                detail,
                error=error,
                operation="create",
            ...
            payload.update(
                {
                    ...
                    "transaction_id": remote_order_id,
                    "msp_cloud_attempt_base": attempt_base,
                    "msp_cloud_attempt_number": attempt_number,
                }
            )
        ...
        try:
            ...
            if not create_response.get_body_success():
                return build_create_error_payload(
    ...
    def _validate_cloud_pos_configuration(self, require_account_key=False):
        ...
        :param bool require_account_key: Whether the merchant account key is needed.
