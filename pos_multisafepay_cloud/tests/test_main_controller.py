...
class TestMainController(HttpCase):
    @patch(
        "odoo.addons.pos_multisafepay_cloud.controllers.main.PosMultiSafepayCloudController._process_post_notification"
    )
    ...
    @patch(
        "odoo.addons.pos_multisafepay_cloud.controllers.main.PosMultiSafepayCloudController._process_get_notification"
    )
...
class TestPosMultiSafepayCloudControllerMethods(TransactionCase):
    def setUp(self):
        ...
        self.mock_env.__getitem__.return_value.sudo.return_value = (
            self.mock_payment_model
        )
    ...
    def test_valid_payload_calls_apply(self, mock_validate):
        ...
        self.mock_payment._build_status_payload.return_value = {
            "status_code": 200,
            "result": "success",
        }
        ...
        self.mock_payment._apply_notification_payload.assert_called_once_with(
            self.payload_with_ref, method="POST"
        )

    def test_get_calls_force_remote_status(self):
        self.mock_payment_model._find_payment.return_value = self.mock_payment
        self.mock_payment._force_remote_status_check.return_value = {
            "status_code": 200,
            "result": "success",
        }
        ...
        self.assertEqual(response.get("status_code"), 200)
        self.mock_payment._force_remote_status_check.assert_called_once_with(
            method="GET"
        )
