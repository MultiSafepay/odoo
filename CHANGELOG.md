...
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and the
version number follows the community convention:
`odoo_major.odoo_minor.module_major.module_minor.module_patch` (e.g. `18.0.1.0.0` = Odoo
18.1 + module v1.0.0), which also aligns with
...
- ODOO-160: Refactor module structure by separating core and enhanced features, reducing
  dependencies
...
- ODOO-134: Floating-point arithmetic calculation error for accurate financial
  calculations
...
- Enhanced logging functionality across multiple modules for better debugging and
  monitoring
