        ...
        for picking in self:
            ...
            if not picking.multisafepay_notified:
                ...
                try:
                    ...
                    carrier_tracking_ref = getattr(picking, 'carrier_tracking_ref', None)
                    carrier_tracking_url = getattr(picking, 'carrier_tracking_url', None)
                    ...
                    if carrier_tracking_ref and picking.date_done:
                        ...
                        if carrier_tracking_url:
                            update_order_request.add_tracktrace_url(carrier_tracking_url)
                        
                        if carrier_id and hasattr(carrier_id, 'name'):
                            update_order_request.add_carrier(carrier_id.name)
                        
                        update_order_request.add_ship_date(picking.date_done.strftime('%Y-%m-%dT%H:%M:%S'))
                        _logger.debug("Including tracking info: %s", carrier_tracking_ref)
                    else:
                        # No tracking info available - notify shipment status only
                        _logger.info("No tracking info available for picking %s (delivery module may not be installed or tracking not set)", 
                                    picking.name)
