...
class PaymentMethod(models.Model):
    ...
    def _get_pricelist_from_context(self, kwargs):
        ...
        3. Current cart from request.cart (e-commerce)
        ...
        if pricelist_id:
            ...
            if pricelist.exists():
                _logger.debug("Pricelist found from explicit pricelist_id: %s", pricelist.name)
                return pricelist
        ...
        if sale_order_id and 'sale.order' in self.env:
            try:
                ...
                if order.exists() and order.pricelist_id:
                    _logger.debug("Pricelist found from sale order %s: %s", order.name, order.pricelist_id.name)
        ...
        try:
            if request and hasattr(request, 'cart') and request.cart and request.cart.exists():
                if request.cart.pricelist_id:
                    _logger.debug("Pricelist found from cart: %s", request.cart.pricelist_id.name)
                    return request.cart.pricelist_id
        ...
        try:
            if request and hasattr(request, 'website') and request.website:
                ...
                if website_pricelist and website_pricelist.exists():
                    _logger.debug("Pricelist found from website: %s", website_pricelist.name)
