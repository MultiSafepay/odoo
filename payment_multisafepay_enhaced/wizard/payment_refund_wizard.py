...
class PaymentRefundWizard(models.TransientModel):
    ...
    def _compute_provider_code(self):
        ...
        for wizard in self:
            wizard.provider_code = (
                wizard.transaction_id.provider_code if wizard.transaction_id else False
            )
    ...
    def action_refund(self):
        ...
        _logger.debug(
            "Refund wizard - Provider code: %s, Reason: %s",
            self.provider_code,
            self.multisafepay_refund_reason,
        )
        ...
        if self.provider_code == "multisafepay" and self.multisafepay_refund_reason:
            _logger.debug(
                "Passing reason through context: %s", self.multisafepay_refund_reason
            )
